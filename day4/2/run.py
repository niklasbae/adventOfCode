...
def onlyTwoChecker(input):
    ...
    x5 = x[4]
    x6 = x[5]
    if(x1 == x2 and x2 != x3) or (x2 == x3 and x1 != x2 and x3 != x4) or (x3 == x4 and x2 != x3 and x4 != x5) or (x4 == x5 and x3 != x4 and x5 != x6) or (x5 == x6 and x4 != x5):
        return True

comb = []

for x in allCombinations:
    if adjacentChecker(x) and risingChecker(x) and onlyTwoChecker(x):
        comb.append(x)
